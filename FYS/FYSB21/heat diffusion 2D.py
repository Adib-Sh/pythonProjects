...
def Heat(x,y,t,a,lim=rmax): #need modification
    ...
    return T0*0.5*(t1-t2)#*np.exp(-a*t)

fig1 = plt.figure()
ax1 = plt.axes(xlim = [0,xlim],
               ylim = [0,ylim])
...
def animate(i):
    global cf
    t = 31536000*i*100000000 #time increment 1million yrs;  seconds in a year: 31536000
    T1 = Heat(X,Y,t,A)
    cf = ax1.imshow(np.flipud(T1+T),extent=[0,8000,0,8000],cmap='jet')
    ax1.title.set_text('Time in hundred million years: %1.1f'  %i)
    cf.set_clim(0,T0)
    return cf
